...
const ContentSection = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  const handleMouseEnter = (img: string, rowId: string) => {
    if (window.innerWidth > 768) {
      ...
      gsap.to(`.service-item-${rowId} .title`, { x: 20, color: "#1f2937", duration: 0.3 }); // gray-800 hex
  ...
  const handleMouseLeave = (rowId: string) => {
    if (window.innerWidth > 768) {
      ...
      gsap.to(`.service-item-${rowId} .title`, { x: 0, color: "#C80000", duration: 0.3 });
  ...
  return (
    <div ref={ref} className="bg-white text-[#C80000] min-h-screen relative py-12 md:py-20 overflow-hidden md:cursor-none">
      ...
      <div ref={containerRef} className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 relative">
        ...
        <div className="mb-12 md:mb-20 border-b border-gray-200 pb-6 md:pb-10 relative z-20">
            <span className="text-gray-800 font-mono tracking-widest text-xs md:text-sm uppercase font-bold block mb-2">
                Our Expertise
            </span>
            <h2 className="section-header-title text-[11vw] md:text-8xl font-bold uppercase tracking-tighter leading-[0.9] text-[#C80000] will-change-transform break-words">
                ...
                >
                    <div className="flex flex-col md:flex-row items-start md:items-baseline justify-between gap-4 md:gap-6 relative z-30 pointer-events-none">
                        ...
                        <div className="flex items-baseline gap-4 md:gap-8 md:w-1/2 w-full">
                            <span className={`font-mono text-lg md:text-xl transition-colors ${activeId === idx.toString() ? 'text-gray-800' : 'text-gray-400'}`}>/{item.id}</span>
                            <h3 className={`title text-2xl sm:text-3xl md:text-5xl font-bold uppercase transition-all duration-300 drop-shadow-sm leading-tight
                              ${activeId === idx.toString() ? 'text-gray-800 translate-x-2' : 'text-[#C80000]'}`}>
                                {item.title}
                            </h3>
                        </div>
                        ...
                        <div className="flex items-center gap-4 md:w-1/2 justify-between w-full pl-10 md:pl-0">
                            ...
                            <span className={`md:hidden text-2xl transition-transform duration-300 ${activeId === idx.toString() ? 'rotate-90 text-gray-800' : 'text-gray-300'}`}>
                                →
                            </span>
                            {/* Desktop Hover Arrow */}
                            <span className="arrow text-gray-800 text-2xl md:text-3xl opacity-0 transform -translate-x-4 transition-all hidden md:block">
                                ↗
                            </span>
                        </div>
                    </div>
