...
const CompanyOverview = () => {
    ...
    return (
        <section ref={containerRef} className="relative w-full min-h-screen bg-white text-[#C80000] pt-10 pb-32 px-4 md:px-6 overflow-hidden cursor-default">
            
            {/* Background Noise adjusted for light background */}
            ...
            <div className="max-w-7xl mx-auto relative z-20">
                <div className="text-center mb-12 md:mb-20">
                    <motion.span 
                        ...
                        className="text-[#C80000] font-mono tracking-[0.2em] text-xs md:text-sm uppercase font-bold block mb-4 md:mb-6"
                    >
                        The Energica Story
                    </motion.span>

                    <h1 className="text-[12vw] md:text-9xl font-black uppercase leading-[0.85] tracking-tighter text-[#C80000]">
                        ...
                        <div className="overflow-hidden">
                            <motion.span 
                                ...
                                className="block"
                            >
                                of <span className="text-[#C80000]/30">Integrity.</span>
                            </motion.span>
                        </div>
                    </h1>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-32 items-start">
                    {content.map((item, index) => (
                        <div 
                            ...
                            className={`group border-t border-[#C80000]/20 pt-8 md:pt-12 ${index === 1 ? 'parallax-col md:mt-40' : ''} relative cursor-pointer md:cursor-default`}
                        >
                            {/* --- IMAGE BACKGROUND --- */}
                            <AnimatePresence>
                                {activeIndex === index && (
                                    ...
                                    >
                                        ...
                                        />
                                        {/* Red-tinted Gradient Overlay for the white theme */}
                                        <div className="absolute inset-0 bg-gradient-to-t from-[#C80000] via-[#C80000]/60 to-[#C80000]/20 mix-blend-multiply"></div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <div className="relative z-10 pointer-events-none md:pointer-events-auto">
                                <div className="flex items-center gap-4 mb-6 md:mb-8">
                                    <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full bg-[#C80000] transition-transform duration-300 ${activeIndex === index ? 'scale-150 bg-white' : ''}`}></div>
                                    
                                    <h3 className={`company-title-${index} text-2xl md:text-4xl font-bold uppercase transition-colors duration-300 ${activeIndex === index ? 'text-white' : 'text-[#C80000]'}`}>
                                        {item.title}
                                    </h3>
                                </div>
                                ...
                                <div>
                                    <MaskedText 
                                        text={item.text} 
                                        className={`text-lg md:text-xl leading-[1.6] transition-colors duration-500 ${activeIndex === index ? 'text-white' : 'text-[#C80000]/70'}`} 
                                    />
                                </div>
                                
                                {/* Mobile Hint */}
                                <div className={`md:hidden mt-4 text-xs font-mono text-[#C80000] transition-opacity duration-300 ${activeIndex === index ? 'opacity-0' : 'opacity-60'}`}>
                                    [ Tap to Reveal ]
                                </div>
                            </div>
                        </div>
