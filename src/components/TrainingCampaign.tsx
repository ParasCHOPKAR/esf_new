...
// --- UPDATED DATA SOURCE WITH IMAGES ---
const solarTech = [
  {
  ...
  return (
    <div ref={container} className="bg-[#050505] text-white min-h-screen relative z-10 overflow-hidden py-24 px-4 md:px-12 font-sans selection:bg-[#28a745] selection:text-black">
      ...
      <div className="max-w-7xl mx-auto relative z-20">
        ...
        {/* --- SECTION 1: SOLAR TECH (Interactive Hover Images) --- */}
        <div className="mb-40">
          ...
          <div className="border-t border-white/20">
            {solarTech.map((item, idx) => (
              <div key={idx} className="spec-row group relative border-b border-white/10 overflow-hidden transition-all duration-700">
                {/* Background Image Reveal on Hover */}
                <div 
                  className="absolute inset-0 opacity-0 group-hover:opacity-20 group-hover:scale-105 transition-all duration-1000 bg-cover bg-center pointer-events-none"
                  style={{ backgroundImage: `url(${item.image})` }}
                />
                
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 py-10 lg:py-16 relative z-10 px-2 lg:px-6">
                  <div className="spec-content lg:col-span-1 font-mono text-[#28a745]/60 text-sm">{`0${idx + 1}`}</div>
                  <div className="spec-content lg:col-span-4">
                    <h3 className="text-2xl md:text-4xl font-bold uppercase group-hover:translate-x-4 transition-transform duration-500">
                      {item.title}
                    </h3>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {item.tags.map((tag, t) => (
                        <span key={t} className="text-[10px] font-mono border border-white/20 px-2 py-1 text-gray-400 group-hover:border-[#28a745] group-hover:text-[#28a745]">{tag}</span>
                      ))}
                    </div>
                  </div>
                  <div className="spec-content lg:col-span-6 lg:col-start-7">
                    <p className="text-gray-400 text-lg leading-relaxed group-hover:text-white transition-colors duration-300">{item.desc}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* --- SECTION 2: NEW TECH (Circuit Flow + Image Nodes) --- */}
        <div className="mb-40">
          <div className="flex items-end justify-between mb-12">
              ...
              <div key={idx} className="spec-row relative py-12 group">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
                  <div className="spec-content lg:col-span-3 lg:text-right">
                    <h3 className="text-xl md:text-2xl font-bold uppercase text-[#28a745] group-hover:brightness-150 transition-all">{item.title}</h3>
                  </div>
                  <div className="hidden lg:flex lg:col-span-1 justify-center relative">
                    <div className="w-3 h-3 bg-[#050505] border border-[#28a745] rounded-full z-10 group-hover:scale-150 transition-transform"></div>
                  </div>
                  <div className="spec-content lg:col-span-8 flex flex-col md:flex-row gap-6 items-center">
                    <p className="text-gray-300 text-lg font-light leading-relaxed flex-1">{item.desc}</p>
                    {/* Small Image Preview */}
                    <div className="w-full md:w-32 h-20 overflow-hidden rounded border border-white/10 group-hover:border-[#28a745]/50 transition-all">
                        <img src={item.image} alt={item.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 scale-110 group-hover:scale-100 transition-all duration-700" />
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* --- SECTION 3: CAPACITY BUILDING (Big Typography with Image Backgrounds) --- */}
        <div className="mb-40">
              ...
              <div key={idx} className="spec-row group relative">
                <div className="flex flex-col md:flex-row gap-6 md:items-baseline relative z-10">
                  <div className="spec-content md:w-1/4">
                    <span className="text-6xl md:text-9xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white/10 to-transparent group-hover:from-[#28a745]/60 transition-all duration-700">
                      0{idx + 1}
                    </span>
                  </div>
                  <div className="spec-content md:w-3/4 border-l border-white/10 pl-6 md:pl-10 group-hover:border-[#28a745] transition-colors duration-500">
                    <h3 className="text-2xl md:text-5xl font-bold uppercase mb-4 text-white group-hover:tracking-widest transition-all duration-500">{item.title}</h3>
                    <p className="text-gray-400 text-xl leading-relaxed max-w-2xl group-hover:text-gray-200">
                      {item.desc}
                    </p>
                  </div>
                </div>
                {/* Large Background Image reveal for Capacity Building */}
                <div 
                  className="absolute right-0 top-0 w-1/3 h-full opacity-0 group-hover:opacity-40 translate-x-10 group-hover:translate-x-0 transition-all duration-1000 pointer-events-none"
                  style={{ 
